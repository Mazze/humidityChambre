...
// void iotc_sendReportedProperty(IOTContextInternal *internal , AzureIOT::StringBuffer& buffer , size_t size )
// {
...
void echoDesired(IOTContextInternal *internal, const char *propertyName,
  ...
  //Serial.println(propertyNameObject.json);  
  char *value = jsobject_get_data_by_name(&rootObject, propertyName);
  double desiredVersion = jsobject_get_number_by_name(&rootObject, "$version");
// Serial.println("echoDesired  done");
//   Serial.printf("echoDesired , desiredVersion: %lf\n", desiredVersion);
//   Serial.println(" ");
...
// \"status\":\"%s\",\"desiredVersion\":%d}}";
  ...
  Serial.println("Free propertyNameObject");

  jsobject_free(&propertyNameObject);
  // Serial.println("Free rootObject");
  jsobject_free(&rootObject);
}
...
void callDesiredCallback(IOTContextInternal *internal,
  ...
  if (internal->callbacks[/*IOTCallbacks::*/ ::SettingsUpdated].callback) {
    ...
    info.statusCode = 200;
    info.callbackResponse = NULL;
    // Serial.println("NOW CALL Callback");
    internal->callbacks[/*IOTCallbacks::*/ ::SettingsUpdated].callback(internal,
                                                                       &info);
    // Serial.println("Back from callback");
    if (topicName.startsWith(
      ...
      if (info.callbackResponse) {
       ...
       Serial.println("Begin echo");
      // Serial.printf("aruments:\n\tpropertyName: %s\n\tpropertyName: %s\n\tpropertyName: %s\n",
      //           propertyName,*payload,response);
      echoDesired(internal, propertyName, payload, response, info.statusCode);
      // Serial.println("Back from echo");
    }
  }
}
...
static void deviceTwinGetStateCallback(AzureIOT::StringBuffer &topicName,
  ...
  memset(&outDesired,0,sizeof(jsobject_t_tag));// AS this is a structure no constucture is there
            // so points are not set to zero. This will cause problems later when you try to 
            // figure if it needs to be free
  memset(&outReported,0,sizeof(jsobject_t_tag));
  ...
  if (jsobject_get_object_by_name(&desired, "desired", &outDesired) != 1 &&
    ...
    // Serial.printf("deviceTwinGetStateCallback 1.a\n");
    callDesiredCallback(internal, topicName, "twin", payload);
    // Serial.println("deviceTwinGetStateCallback 1.b\n");    
  } else {
    // Serial.printf("deviceTwinGetStateCallback 2.a\n");
    for (unsigned i = 0, count = jsobject_get_count(&desired); i < count;
      ...
      // else
      // { 
      //   Serial.printf("Got a null \n");
        
      // }
      if (itemName != NULL && itemName[0] != '$') {
        callDesiredCallback(internal, topicName, itemName, payload);
      }
      // Serial.printf("deviceTwinGetStateCallback 2.b\n");
      if (itemName) IOTC_FREE(itemName);
      // Serial.printf("deviceTwinGetStateCallback 2.c\n");
    }
  }
  // Serial.printf("deviceTwinGetStateCallback 2\n");
  jsobject_free(&desired);
  // Serial.printf("deviceTwinGetStateCallback 3\n");
  jsobject_free(&outReported);
  // Serial.println("deviceTwinGetStateCallback 4\n");
  jsobject_free(&outDesired);
  // Serial.println("deviceTwinGetStateCallback 5\n");
}
...
int iotc_send_property(IOTContext ctx, const char *payload, unsigned length) {
  ...
  if (mqtt_publish(internal, *topic, topic.getLength(), payload, length) != 0) {
    IOTC_LOG("ERROR: (iotc_sendReportedProperty) MQTTClient publish has failed => %s",
             payload);
    return 1;
  }
